...
class A_FeatureImageCacheAddon extends Autoblog_Addon_Image {
	...
	public function __construct() {
		...
		$this->_add_action( 'admin_enqueue_scripts', 'enqueue_media_picker', 10, 1 );
	}

	/**
	 * Enqueues media picker for featured image selection.
	 *
	 ...
	 */
	public function enqueue_media_picker( $page_hook ) {
		// Only load on the feeds admin page
		if ( $page_hook === 'toplevel_page_autoblog' || $page_hook === 'rss-autoblog_page_autoblog_admin' || strpos( $page_hook, 'autoblog_admin' ) !== false ) {
			wp_enqueue_media();
		}
	}
	...
	public function render_image_options( $key, $details ) {
		...
		$options         = array(
			''                           => __( "Importiere keine vorgestellten Bilder", 'autobogtext' ),
			self::SOURCE_MEDIA_THUMBNAIL => __( 'Medien verwenden: Miniaturansicht eines Feedelements', 'autoblogtext' ),
			self::SOURCE_ENCLOSURE       => __( 'Verwende das Basis-Tag eines Feed-Elements', 'autoblogtext' ),
			self::SOURCE_THE_FIRST_IMAGE => __( 'Suche das erste Bild im Inhalt eines Feedelements', 'autoblogtext' ),
			self::SOURCE_THE_LAST_IMAGE  => __( 'Suche das letzte Bild im Inhalt eines Feedelements', 'autoblogtext' ),
		);
		...
		$default = sprintf(
			...
			$thumbnail_id,
			__( 'Wähle Standardbild', 'autoblogtext' ),
			__( 'Lösche Bild', 'autoblogtext' ),
			$thumbnail_src
		);
		...
		$this->_render_block_element( __( 'Standard-Miniaturbild', 'autoblogtext' ), $default );
	}
