...
/*
Addon Name: Bilder entaggen
Description: Entfernt alle Bild-Tags aus dem Beitragsinhalt.
Author: PSOURCE
...
class A_StripImagesAddon extends Autoblog_Addon
{
    ...
    public function add_feed_option($key, $details)
    {
        ...
        // render block header
        $this->_render_block_header(esc_html__('Bilder entaggen', 'autoblogtext'));

        // render block elements
        $this->_render_block_element(esc_html__('Bild-Tags entfernen', 'autoblogtext'), sprintf(
            '<input type="checkbox" name="abtble[stripimgtags]" value="1"%s>',
            checked(isset($table['stripimgtags']) && $table['stripimgtags'] == '1', true, false)
        ));

        $this->_render_block_element(esc_html__('Ersetzen mit', 'autoblogtext'), sprintf(
