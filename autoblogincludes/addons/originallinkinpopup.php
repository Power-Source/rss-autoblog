...
/*
Addon Name: Öffne Links im Popup
Description: Wenn Dein Leser auf Links aus der Quelle klickt, werden diese in einem Popup geöffnet
Author: PSOURCE
...
EOP;
	...
	public function add_footer_options( $key, $details ) {
		...
		$label = sprintf( '<p>%s</p><p>%s</p>',
			__( 'Möchtest Du diese Funktion für diesen Feed deaktivieren?', 'autoblogtext' ),
			__( 'Domain, die Du ausschließen möchtest, getrennt nach Zeilen (ohne "http(s)://")', 'autoblogtext' ));

		$content = sprintf( '<p>%s</p><p>%s</p>',
			'<label><input ' . checked( @$data['olp_disable'], 'on', false ) . ' name="abtble[olp_disable]" type="checkbox">' . __( 'Ja','autoblogtext' ) . '</label>',
			'<textarea name="abtble[olp_domain_exclude]" rows="3" class="long field">' . @$data['olp_domain_exclude'] . '</textarea>'
		);
		$this->_render_block_header( __( 'Öffne den Link im Popup', 'autoblogtext' ) );
