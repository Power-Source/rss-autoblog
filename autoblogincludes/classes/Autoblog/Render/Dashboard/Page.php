...
class Autoblog_Render_Dashboard_Page extends Autoblog_Render {
	...
	protected function _to_html() {
		?><div class="wrap">
			...
			<h2>
				...
				<a class="add-new-h2" href="<?php echo esc_url( $this->export_log_url ) ?>"><?php esc_html_e( 'Protokoll exportieren', 'autoblogtext' ) ?></a>
				<a class="add-new-h2" href="<?php echo esc_url( $this->clear_log_url ) ?>" onclick="return confirm('<?php esc_html_e( 'Möchtest Du wirklich die Protokollsätze löschen?', 'autoblogtext' ) ?>')">
					<?php esc_html_e( 'Protokoll löschen', 'autoblogtext' ) ?>
				</a>
			</h2>
	...
	private function _render_empty_message() {
		?><div id="autoblog-empty-box">
			<h1><?php esc_html_e( 'Es wurden keine Protokolldatensätze gefunden.', 'autoblogtext' ) ?></h1>
		</div><?php
	}
	...
	private function _render_feed_errros_info( $feed ) {
		...
		?><div class="autoblog-log-feed-info autoblog-log-feed-errors<?php echo $class ?>" title="<?php esc_attr_e( 'Die Anzahl der Fehler', 'autoblogtext' ) ?>">
	...
	private function _render_feed_iterations_info( $feed ) {
		...
		?><div class="autoblog-log-feed-info autoblog-log-feed-iterations<?php echo $class ?>" title="<?php esc_attr_e( 'Die Häufigkeit, mit der die Feeds verarbeitet wurden', 'autoblogtext' ) ?>">
	...
	private function _render_feed_imports_info( $feed ) {
		...
		?><div class="autoblog-log-feed-info autoblog-log-feed-imports<?php echo $class ?>" title="<?php esc_attr_e( 'Die Menge der importierten Artikel während des Tages.', 'autoblogtext' ) ?>">
	...
	private function _render_log_row( $log ) {
		...
		switch ( $log['log_type'] ) {
			case Autoblog_Plugin::LOG_INVALID_FEED_URL:
				$glyph = 'warning-sign';
				$message = esc_html__( 'Die Feed-URL ist ungültig und kann nicht verarbeitet werden.', 'autoblogtext' );
				break;
			...
			case Autoblog_Plugin::LOG_DUPLICATE_POST:
				...
				$message = sprintf(
					esc_html_x( '%s wurde bereits importiert.', '{Post title} wurde bereits importiert.', 'autoblogtext' ),
					sprintf( '<a href="%s" target="_blank"><b>%s</b></a>', esc_url( $info[$info['checked']] ), esc_html( $info['title'] ) )
				);

				$permalink = get_permalink( $info['post_id'] );
				if ( $permalink ) {
					$message .= sprintf( ' <a href="%s" target="_blank"><small>(%s)</small></a>', $permalink, esc_html__( 'Beitrag anzeigen', 'autoblogtext' ) );
				}
				break;

			case Autoblog_Plugin::LOG_POST_DOESNT_MATCH:
				$glyph = 'info-sign';
				$message = esc_html__( 'Das Feedelement enthält keine angeforderten Wörter oder Tags.', 'autoblogtext' );
				break;

			case Autoblog_Plugin::LOG_POST_INSERT_FAILED:
				$glyph = 'warning-sign';
				$message = esc_html__( 'Import von Feedelementen fehlgeschlagen.', 'autoblogtext' );
				break;
			...
			case Autoblog_Plugin::LOG_POST_INSERT_SUCCESS:
				...
				$info = unserialize( $log['log_info'] );
				$message = sprintf(
					esc_html_x( '%s wurde erfolgreich importiert.', '{Post title} wurde erfolgreich importiert.', 'autoblogtext' ),
			...
			case Autoblog_Plugin::LOG_POST_UPDATE_SUCCESS:
				...
				$info = unserialize( $log['log_info'] );
				$message = sprintf(
					esc_html_x( '%s wurde erfolgreich aktualisiert.', '{Post title} wurde erfolgreich aktualisiert.', 'autoblogtext' ),
					sprintf( '<a href="%s" target="_blank"><b>%s</b></a>', esc_url( $info['link'] ), esc_html( $info['title'] ) )
				);
			...
			case Autoblog_Plugin::LOG_FEED_PROCESSED:
				...
				$message = sprintf(
					_n( '%s Feedelement wurde importiert.', '%s Feedelemente wurden importiert.', $imported, 'autoblogtext' ),
			...
			case Autoblog_Plugin::LOG_FEED_SKIPPED_TOO_EARLY:
				$glyph = 'info-sign';
				$message = esc_html__( 'Der Feed wurde übersprungen, da es zu früh ist, ihn zu verarbeiten.', 'autoblogtext' );
				break;

			case Autoblog_Plugin::LOG_FEED_SKIPPED_TOO_LATE:
				$glyph = 'info-sign';
				$message = esc_html__( 'Der Feed wurde übersprungen, da es zu spät ist, ihn zu verarbeiten.', 'autoblogtext' );
				break;

			case Autoblog_Plugin::LOG_FEED_PROCESSED_NO_RESULTS:
				$glyph = 'info-sign';
				$message = esc_html__( 'Die Feedverarbeitung wurde beendet. Es wurden keine neuen Artikel importiert.', 'autoblogtext' );
				break;
		}
	...
	public function get_html_from_cahce() {
		...
		if ( $cache !== false ) {
			...
			if ( $expire !== false ) {
				...
				$cache .= sprintf(
					_x( '* Diese Seite wurde aus dem Cache entfernt und wird am %s um %s neu generiert.', '... und wird regeneriert um {14:01:54} am {Wednesday, 25-Dec-13}.', 'autoblogtext' ),
					date( get_option( 'time_format' ), $expire ),
					date( get_option( 'date_format' ), $expire )
				);
			} else {
				$cache .= __( '* Diese Seite wurde aus dem Cache entfernt.', 'autoblogtext' );
			}

			$cache .= sprintf( ' <a href="%s">%s</a>', esc_url( $this->regenerate_url ), __( 'Generiere die Seite neu.', 'autoblogtext' ) );
			$cache .= '</div>';
		}
