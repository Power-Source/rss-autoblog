...
class Autoblog_Table_Addons extends Autoblog_Table {
	...
	public function column_addon( $item ) {
		...
		if ( in_array( $item['Source'], $this->_args['oposite'] ) ) {
			$actions[] = sprintf( '<a class="disabled" href="#">%s</a>', __( 'Netzwerkweit aktiv', 'autoblogtext' ) );
		} else {
			if ( in_array( $item['Source'], $this->_args['active'] ) ) {
				$actions['Deaktivieren'] = sprintf(
					'<a href="%s">%s</a>',
					add_query_arg( array(
						'action'               => 'deactivate',
						'noheader'             => 'true',
	...
	public function get_columns() {
		return array(
			...
			'addon'       => __( 'Erweiterung', 'autoblogtext' ),
			'description' => __( 'Beschreibung', 'autoblogtext' ),
		);
	}
