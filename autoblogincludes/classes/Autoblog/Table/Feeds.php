...
class Autoblog_Table_Feeds extends Autoblog_Table {
	...
	public function column_title( $item ) {
		$title = !empty( $item['feed_meta']['title'] )
			...
			: esc_html__( 'unbekannt', 'autoblogtext' );
	...
	public function column_post_type( $item ) {
		if ( empty( $item['feed_meta']['posttype'] ) ) {
			return esc_html__( 'unbekannt', 'autoblogtext' );
		}
	...
	public function column_lastupdated( $item ) {
		if ( $item['lastupdated'] == 0 ) {
			return '<code>' . __( 'Niemals', 'autoblogtext' ) . '</code>';
		}
	...
	public function get_columns() {
		...
		if ( is_network_admin() ) {
			$columns['blogname']    = __( 'Ziel Blog', 'autoblogtext' );
		}

		$columns['post_type'] = __( 'Beitrags-Typ', 'autoblogtext' );
		$columns['lastupdated'] = __( 'Zuletzt verarbeitet', 'autoblogtext' );
		$columns['nextcheck']   = __( 'Nächster Check', 'autoblogtext' );
	...
	private function _convert_time_to_str( $timestamp ) {
		...
		if ( $diff > 0 ) {
			...
			if ( $day_diff == 0 ) {
				if ( $diff < MINUTE_IN_SECONDS ) {
					return __( 'Jetzt gerade', 'autoblogtext' );
				}

				if ( $diff < 2 * MINUTE_IN_SECONDS ) {
					return __( 'Vor 1 Minute', 'autoblogtext' );
				}

				if ( $diff < HOUR_IN_SECONDS ) {
					return floor( $diff / MINUTE_IN_SECONDS ) . __( ' Minuten zuvor', 'autoblogtext' );
				}

				if ( $diff < 2 * HOUR_IN_SECONDS ) {
					return __( 'vor 1 Stunde', 'autoblogtext' );
				}

				if ( $diff < DAY_IN_SECONDS ) {
					return floor( $diff / HOUR_IN_SECONDS ) . __( ' Stunden zuvor', 'autoblogtext' );
				}
			}

			if ( $day_diff == 1 ) {
				return __( 'Gestern', 'autoblogtext' );
			}

			if ( $day_diff < 7 ) {
				return $day_diff . __( ' Tage zuvor', 'autoblogtext' );
			}

			if ( $day_diff < 31 ) {
				return ceil( $day_diff / 7 ) . __( ' Wochen zuvor', 'autoblogtext' );
			}

			if ( $day_diff < 60 ) {
				return __( 'letzten Monat', 'autoblogtext' );
			}
		...
		if ( $day_diff == 0 ) {
			if ( $diff < 2 * MINUTE_IN_SECONDS ) {
				return __( 'in einer Minute', 'autoblogtext' );
			}

			if ( $diff < HOUR_IN_SECONDS ) {
				return __( 'in ', 'autoblogtext' ) . floor( $diff / MINUTE_IN_SECONDS ) . __( ' Minuten', 'autoblogtext' );
			}

			if ( $diff < 2 * HOUR_IN_SECONDS ) {
				return __( 'in einer Stunde', 'autoblogtext' );
			}

			if ( $diff < DAY_IN_SECONDS ) {
				return __( 'in ', 'autoblogtext' ) . floor( $diff / HOUR_IN_SECONDS ) . __( ' Stunden', 'autoblogtext' );
			}
		}
		if ( $day_diff == 1 ) {
			return __( 'Morgen', 'autoblogtext' );
		}
		...
		if ( $day_diff < 7 + ( 7 - date( 'w' ) ) ) {
			return __( 'nächste Woche', 'autoblogtext' );
		}

		if ( ceil( $day_diff / 7 ) < 4 ) {
			return __( 'in ', 'autoblogtext' ) . ceil( $day_diff / 7 ) . __( ' Wochen', 'autoblogtext' );
		}

		if ( date( 'n', $timestamp ) == date( 'n' ) + 1 ) {
			return __( 'nächsten Monat', 'autoblogtext' );
		}
