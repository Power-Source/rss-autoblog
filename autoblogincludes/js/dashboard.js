...
(function($) {
	$(function() {
		$('.autoblog-log-feed > .autoblog-log-row').on('click', function() {
			...
			parent.find('.autoblog-log-feed-records-container').toggle();

			rows = parent.find('.autoblog-log-feed-records .autoblog-log-record');
			height = rows.length * $(rows[0]).outerHeight();
			if (height < 400) {
				parent.find('.autoblog-log-feed-records-container, .autoblog-log-feed-records').height(height + 'px');
			} else {
				parent.find('.autoblog-log-feed-records-container, .autoblog-log-feed-records').height('400px');
			}
		...
		$('.autoblog-log-feed-records-container').height('400px').show();
